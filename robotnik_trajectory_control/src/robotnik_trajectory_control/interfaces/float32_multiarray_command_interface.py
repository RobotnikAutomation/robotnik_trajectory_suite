...
#		
class Float32MultiArrayCommandInterface(DeviceCommandInterface):
	'''
	...
	def setup(self):
		...
		self.command.data = [0] * len(self.joint_names)
		
		if len(self.command_topic) > 0:			
			try:
				self.command_publisher = rospy.Publisher(self.command_topic, Float32MultiArray)
	...
	def sendCommand(self):
		...
		for i in range(len(self.joint_names)):
			...
			self.command.data[i] = self.joint_state_pointer[self.joint_names[i]][0]
